...
*/
    if (s == NULL) { //Nedostali jsme ukazatel na platnou strukturu, zavolame stackError
        stackError(SERR_INIT);
    } else {
        s->top = -1; //Inicializujeme prázdný zásobník, vrchol bude tedy null
    }
}
...
*/
    return (s->top != -1) ? 0
                          : 1; //Pokud je zásobník prázdný vrátíme nulu, jinak nenulovou hodnotu
}
...
*/
    return ((STACK_SIZE - 1) != s->top) ? 0
                                        : 1; //Pokud je vrchol zásobníku roven velikosti zásobníku (-1), pak je zásovník plný a vracíme nenulovou hodnotu, jinak vracíme nulu.
}
...
*/
    ...
    if (stackEmpty(s) == 0) { //Pouze, pokud neni zasobnik prazdny
        *c = s->arr[s->top]; //Do proměnné c uložíme znak z vrcholu zásobníku
    } else { //Zasovnik je prazdny, zavolame stackError
        stackError(SERR_TOP);
    }
}
...
*/
    ...
    if (stackEmpty(s) == 0) { //Pouze, pokud neni zasobnik prazdny
        s->arr[s->top] = '\0'; //Odstraneni znaku ze zásobníku
        s->top -= 1; //Ukazatel na vrchol posuneme o smazany prvek
    }
}
...
*/
    ...
    if (stackFull(s) == 0) { //Pouze, pokud není zásobník plný
        s->top += 1;
        s->arr[s->top] = c;
    } else {
        stackError(SERR_PUSH); //Zásobík je již plný, zavoláme stackError
    }
}
